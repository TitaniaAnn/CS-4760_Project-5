...
for i in $(seq 1 8); do
    sleep 0.5
    COUNT=$(pgrep -P $OSS_PID 2>/dev/null | wc -l)
...
FORKED=$(grep "forked worker" t07.log 2>/dev/null | wc -l)
...
cleanup_ipc
BLOCKED=$(grep "not granted" t09.log 2>/dev/null | wc -l)
UNBLOCKED=$(grep "from freed resources" t09.log 2>/dev/null | wc -l)
...
cleanup_ipc
DL_RUNS=$(grep "running deadlock detection" t10.log 2>/dev/null | wc -l)
...
RC=$?
FORKED=$(grep "forked worker" t14.log 2>/dev/null | wc -l)
...
DL_KILLS=${DL_KILLS:-0}
DL_RUNS=$(grep "running deadlock detection" t15.log 2>/dev/null | wc -l)
...
RC=$?
FORKED=$(grep "forked worker" t16.log 2>/dev/null | wc -l)
if [ "$RC" -eq 0 ] && [ "$FORKED" -ge 1 ] && ipc_clean; then
...
RC=$?
FORKED=$(grep "forked worker" t17.log 2>/dev/null | wc -l)
...
RC=$?
FORKED=$(grep "forked worker" t18.log 2>/dev/null | wc -l)
if [ "$RC" -eq 0 ] && [ "$FORKED" -ge 1 ] && ipc_clean; then
...
RC=$?
FORKED=$(grep "forked worker" t19.log 2>/dev/null | wc -l)
...
RC=$?
FORKED=$(grep "forked worker" t20.log 2>/dev/null | wc -l)
...
if [ "$ELAPSED" -ge 25 ] && [ "$ELAPSED" -le 38 ] && ipc_clean && [ "$ORPHANS" -eq 0 ]; then
    FORKED=$(grep "forked worker" t21.log 2>/dev/null | wc -l)
